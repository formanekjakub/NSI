...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///database.db'  # connection to the database
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
app.config['SECRET_KEY'] = os.urandom(24)  # used only for logging out when the application stops

db.init_app(app)

# Create tables immediately after initializing the application
...
login_manager.login_view = 'login'  # where to redirect an unauthenticated user
...
def register():
    if request.method == 'POST':
        ...
        existing_user = User.query.filter_by(username=username).first()  # load the last user into the variable
        if existing_user:
